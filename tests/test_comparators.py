...
def _dummy_reps(m, nx, ny):
    x = np.random.randn(m, nx) + np.abs(np.random.randn(1, nx))
    y = np.random.randn(m, ny) + np.abs(np.random.randn(1, ny))
    return x, y
...
class TestPrepReps(unittest.TestCase):
    def test_no_center_no_scale(self):
        ...
        npt.assert_raises(AssertionError, np.testing.assert_array_equal, x, x2)
        npt.assert_raises(AssertionError, np.testing.assert_array_equal, y, y2)
    ...
    def test_center_no_scale(self):
        ...
        npt.assert_raises(
            AssertionError, np.testing.assert_array_equal, x.mean(axis=0), np.zeros(10)
        )
        npt.assert_raises(
            AssertionError, np.testing.assert_array_equal, y.mean(axis=0), np.zeros(10)
        )

        # Assert x2 and y2 *do* have mean zero
        npt.assert_array_almost_equal(x2.mean(axis=0), np.zeros(10))
        npt.assert_array_almost_equal(y2.mean(axis=0), np.zeros(10))

    def test_center_m_equals_n(self):
        # Checking the 'keepdims' aspect of centering
    ...
    def test_no_center_scale(self):
        ...
        npt.assert_almost_equal(np.linalg.norm(x2, ord="fro"), 1.0)
        npt.assert_almost_equal(np.linalg.norm(y2, ord="fro"), 1.0)
...
class TestDoubleCenter(unittest.TestCase):
    def test_linear_kernel(self):
        ...
        gram_x = np.einsum("in,jn->ij", x, x)
        centered_gram_x = double_center(gram_x)

        centered_x = x - x.mean(axis=0, keepdims=True)
        gram_centered_x = np.einsum("in,jn->ij", centered_x, centered_x)

        npt.assert_array_almost_equal(centered_gram_x, gram_centered_x)
...
class TestMetricRelations(unittest.TestCase):
    ...
    def _test_metrics_agree_helper(self, metric1: Callable, metric2: Callable):
        ...
        npt.assert_raises(AssertionError, npt.assert_almost_equal, similarity1, 0.0)
        npt.assert_raises(AssertionError, npt.assert_almost_equal, similarity2, 0.0)

        # Assert the two methods agree
        npt.assert_almost_equal(similarity1, similarity2)
